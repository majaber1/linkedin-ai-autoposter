...
function normalizeDatabaseUrl(value = '') {
  return String(value).replace(/([?&])sslmode=(prefer|require|verify-ca)(&|$)/i, '$1sslmode=verify-full$3');
}
  ...
  return {
    ...
    async claimPostForPublishing(id) {
      ...
      try {
        return await this.updatePost(id, {
          ...
          publishingAttempts: [...(current.publishingAttempts || []), { startedAt: new Date().toISOString(), status: 'publishing' }].slice(-50)
        });
      } catch (error) {
    ...
    async finishPublishing(id, changes) {
      try {
        ...
        return await this.updatePost(id, {
          ...changes,
          publishingAttempts: [...(current.publishingAttempts || []), {
            finishedAt: new Date().toISOString(), status: changes.status, error: changes.error || null,
...
function postgresDriver() {
  ...
  const rowToPost = row => row && ({
    ...
    publishingAttempts: row.publishing_attempts || []
  });
  ...
  return {
    ...
    async init() {
      const schema = `
        ...
        ALTER TABLE posts ADD COLUMN IF NOT EXISTS revisions JSONB NOT NULL DEFAULT '[]'::jsonb;
        ALTER TABLE posts ADD COLUMN IF NOT EXISTS publishing_attempts JSONB NOT NULL DEFAULT '[]'::jsonb;
    ...
    async savePost(record) {
      await pool.query(
        `INSERT INTO posts (id, title, text, status, provider, topic_slug, created_at, updated_at, published_at, linkedin_id, scheduled_for, language, brief, revisions, publishing_attempts)
        ...
        [assertPostId(record.id), record.title, record.text, record.status, record.provider,
          ...
          JSON.stringify(record.revisions || []), JSON.stringify(record.publishingAttempts || [])]
      );
      return record;
    },
    ...
    async updatePost(id, changes) {
      ...
      const columns = {
        ...
        revisions: 'revisions', publishingAttempts: 'publishing_attempts'
      };
      const sets = [];
      const values = [assertPostId(id)];
      const jsonColumns = new Set(['brief', 'revisions', 'publishingAttempts']);
      for (const [key, column] of Object.entries(columns)) {
        if (changes[key] !== undefined) {
          values.push(jsonColumns.has(key) ? JSON.stringify(changes[key]) : changes[key]);
    ...
    async claimPostForPublishing(id) {
      const { rows } = await pool.query(
        `UPDATE posts SET status = 'publishing', error = NULL, updated_at = now(),
           publishing_attempts = publishing_attempts || jsonb_build_array(jsonb_build_object('startedAt', now(), 'status', 'publishing'))
...
module.exports = { store, filesystemDriver, normalizeDatabaseUrl, assertPostId, POST_ID_PATTERN };
