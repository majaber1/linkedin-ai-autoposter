...
function filesystemDriver() {
  ...
  return {
    ...
    async listDuePosts(now = new Date()) {
      ...
      return all.filter(post => post.status === 'approved' && post.scheduledFor && new Date(post.scheduledFor) <= now);
    },

    async claimPostForPublishing(id) {
      if (publishingClaims.has(id)) return null;
      const current = await this.getPost(id);
      if (!['draft', 'approved', 'failed'].includes(current.status)) return null;
...
function postgresDriver() {
  ...
  return {
    ...
    async savePost(record) {
      await pool.query(
        `INSERT INTO posts (id, title, text, status, provider, topic_slug, created_at, updated_at, published_at, linkedin_id, scheduled_for, language)
        ...
        [assertPostId(record.id), record.title, record.text, record.status, record.provider,
          ...
          record.scheduledFor || null, record.language || null]
      );
      return record;
    },
    ...
    async listDuePosts(now = new Date()) {
      const { rows } = await pool.query(
        `SELECT * FROM posts WHERE status = 'approved' AND scheduled_for IS NOT NULL AND scheduled_for <= $1
    ...
    async claimPostForPublishing(id) {
      const { rows } = await pool.query(
        `UPDATE posts SET status = 'publishing', error = NULL, updated_at = now()
         WHERE id = $1 AND status IN ('draft', 'approved', 'failed') RETURNING *`, [assertPostId(id)]
      );
